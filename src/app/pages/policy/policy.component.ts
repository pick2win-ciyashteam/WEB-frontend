...
export class PolicyComponent {
  ...
  readonly sections: PolicySection[] = [
    ...
    { title: 'Payments', body: 'All payments are processed by <strong>Stripe</strong>. We never see or store full card numbers or CVV - that data goes directly to Stripe under PCI-DSS. We retain only the minimum record (pack, amount, date, reference) needed for billing and refunds.' },
    { title: 'Sharing with processors', body: 'We share the minimum data required with service providers acting on our behalf: Stripe (payments), our SMS / email provider (verification), and our football-data provider (lineups). These providers are bound by contract and may not use your data for their own purposes.' },
