...
export class AdminsTeamComponent implements OnInit, AfterViewInit {
  primary = { name: '', email: '', mobile: '', twoFactor: false };
  ...
  loadProfile(): void { this.adminService.getAdminProfile().subscribe({ next: res => { const profile = res?.data || res; if (profile?.id) this.primary = { ...this.primary, ...profile, twoFactor: Number(profile.twofa_enabled ?? profile.two_factor_enabled ?? 0) === 1 }; }, error: err => this.show(err?.error?.message || 'Unable to load administrator profile.', true) }); }
  ...
  toggle2Fa(): void { this.primary.twoFactor = !this.primary.twoFactor; this.adminService.toggleAdmin2Fa(this.primary.twoFactor).subscribe({ next: res => this.show(res?.message || 'Two-factor authentication updated.'), error: err => { this.primary.twoFactor = !this.primary.twoFactor; this.show(err?.error?.message || 'Unable to update 2FA.', true); } }); }
